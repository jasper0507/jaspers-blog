...
`publish` 展示并提交写作内容（`posts/`、`shuoshuo/`、`about.md`、`post-next-id.json`）的新增、修改和删除，不夹带入口脚本或配置，不修改 `draft`。无新内容改动时不制造空提交，改为重新触发远端发布。推送成功不等于上线：命令关联本次内容提交或重试对应的工作流任务，按校验、构建、部署报告结果。本机需已安装并登录 GitHub CLI。

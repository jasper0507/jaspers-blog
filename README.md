...
| 设置区      | 可配置内容                                                     | 空值含义                                                                                           |
| :---------- | :------------------------------------------------------------- | :------------------------------------------------------------------------------------------------- |
| `site`      | 博客名称、可选页头短名称、正式网址、默认简介和可选浏览器图标   | 名称、网址、简介必填；`headerTitle` / `favicon` 省略或空字符串视为省略（短名回退站名，无图标标签） |
| `author`    | 作者显示名，以及页脚固定展示的 GitHub 个人主页和邮箱           | 三项均必填                                                                                         |
| `home.hero` | 首页文案、必需的亮色主视觉、可选暗色主视觉和图片说明           | caption、亮图必填；`darkImage` 省略或空字符串视为省略（复用亮图）；`alt` 可填空字符串表示装饰图    |
| `footer`    | 左侧纯文本，可留空；支持 `{year}`、`{author}`，不支持 Markdown | 空或只含空格则不渲染左侧                                                                           |

页脚右侧仍是固定的 RSS、GitHub 与邮箱。页头短名称只用于顶栏。

设置文件通过 TypeScript 检查字段和类型；必填空值、缺失的本地图片/图标、缺失的「关于我」文件，以及非法网址，会在 `npm run check` 或构建时以中文失败。网址仍由原生 `URL` 解析，不另做 HTTPS 或域名根校验。每个字段的必填/可省略/可空已标在 [`blog.config.ts`](blog.config.ts) 注释里。

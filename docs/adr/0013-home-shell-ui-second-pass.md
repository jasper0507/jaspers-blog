...
归档页 **打开即见内容**：无可见栏目大标题、intro 与顶部分割装饰；保留 document title 与 `sr-only` 页面名「归档」。已发布技术文章按年分组，以 **卡片时间轴** 展示：竖线与圆点共用同一横坐标并对齐穿过；每条卡片含 `YYYY-MM-DD` 日期、标题链到 `/posts/<稳定ID>/`、以及可点标签（复用单篇页标签组件；无标签不渲染该行）；不展示分类路径与摘要。草稿不出现。验收落在 dist 合同与 visual（含轴线–圆点对齐）上。

## 标签（#14 已落地）

标签索引 **打开即见标签云**：无可见栏目大标题与 intro；保留 document title 与 `sr-only` 页面名「标签」。云条目为 **中性纸面 chip 卡**（`surface` 底 + 细边框 + 圆角）：标签名 + 极淡计数角标；全 chip **同字号**，权重只靠计数。排布为内容宽 flex 换行。悬停：`scale(1.04)` + 边框略染 accent + 轻阴影（与归档卡同语言）。仅美化 `/tags/` 索引。单篇/归档上的挂文标签形态已由 **ADR-0014 / #21** 改为弱化 `#tag` 文本链（本段原「轻量 pill」不再适用）。点击进入该标签下的技术文章列表：同样无栏目壳；条目为 `YYYY-MM-DD` 日期 + 标题链到 `/posts/<稳定ID>/`，不展示摘要。仅聚合已发布技术文章；无任何标签时展示「暂无标签」空态。说说不参与标签。验收落在 dist 合同与 visual 上；人工验收通过（含 chip 美化 `2329ada`）。

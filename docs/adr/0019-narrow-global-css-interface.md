...
2026-09-12 修订：搜索不再由 `BaseLayout` / `global.css` 所有。外壳只在主导航提供位置；Pagefind 装配、可用状态、主题映射与 live region 补丁改由搜索 module 所有（ADR-0032）。同日：单篇技术文章的标题、正文目标、目录、回顶与 `--reading-width` 改由阅读 module 所有（ADR-0033）。
...
- **页面**：首页、标签索引、标签详情、归档、说说、关于各自拥有其排版与视觉；单篇技术文章页拥有主区壳层。页面响应式规则跟随页面 owner。页面直接渲染的 DOM 优先使用同文件 scoped style，跨子 module 的情境布局才在页面内使用最小 global selector。
- **可复用 module**：`MarkdownBody`、`PostTags` 各自拥有内部视觉。`--code-*` 只属于 `MarkdownBody`。
- **技术文章阅读**：标题、正文目标、目录位置、滚动与焦点由阅读 module 所有；目录与回顶是其内部 implementation。`--reading-width` 属于该 module，不再作为全站 token。

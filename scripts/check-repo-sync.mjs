...
try {
  ...
  try {
    ...
    await assert.rejects(alignMain(workingDirectory, "publish"), {
      message: userMessage("publish", ERROR_CODE.CONFLICT),
    });
  ...
  await assert.rejects(alignMain(workingDirectory, "create"), {
    message: userMessage("create", ERROR_CODE.NOT_ON_MAIN),
  });
  await git(workingDirectory, ["switch", "main"]);

  await git(workingDirectory, ["remote", "remove", "origin"]);
  await assert.rejects(alignMain(workingDirectory, "publish"), {
    message: userMessage("publish", ERROR_CODE.NO_ORIGIN),
  });
  ...
  await assert.rejects(alignMain(workingDirectory, "create"), {
    message: userMessage("create", ERROR_CODE.NETWORK),
  });
} finally {

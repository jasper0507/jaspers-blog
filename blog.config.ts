...
export default {
  ...
  site: {
    ...
    title: "Jasper's Blog",
    /** 【可省略；空字符串视为省略】页头短名称，只用于顶栏；省略或留空时使用博客名称。建议不超过 16 个字符。 */
    headerTitle: "JaspersBlog",
    /** 【必填】正式网址：建议填写 HTTPS 域名根地址。构建时由原生 URL 解析，非法网址会中文报错。 */
    url: "https://jasper0507.me",
    /** 【必填】默认简介：用于没有单独简介的页面元信息和 RSS。 */
    description: "Jasper 的个人技术博客，记录技术文章与说说。",
    /** 【可省略；空字符串视为省略】浏览器图标。推荐 `/favicon.svg`（放在 public/ 根上）；也可指向 public/ 内其它 svg、png、ico。文件必须存在。 */
    favicon: "/favicon.svg",
  },
  ...
  home: {
    hero: {
      ...
      /** 【可省略；空字符串视为省略】暗色主视觉；应与亮图尺寸及主体位置一致。省略或留空时复用亮图。路径规则同亮图。 */

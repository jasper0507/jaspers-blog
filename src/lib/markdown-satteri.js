...
function renderKatex(source, displayMode) {
  try {
    ...
    try {
      ...
      return `<span class="katex-error" title="${escapeHtml(String(error))}" style="color:#cc0000">${escapeHtml(source)}</span>`;
    }
  }
}
        ...
        children: [
          {
            type: "element",
            tagName: "path",
            properties: { d: alertIcons[type] },
            children: [],
          },
        ],
      },
      { type: "text", value: type.toUpperCase() },
    ],
  };
}

function alertBody(blockquote, firstParagraph, marker, rest) {
  const firstChildren = [];
  for (const child of firstParagraph.children ?? []) {
    if (child === marker) {
      if (rest) firstChildren.push({ type: "text", value: rest });
      continue;
    }
...
const githubAlertPlugin = {
  name: "site-github-alerts",
  element: {
    filter: ["blockquote"],
    visit(node) {
      const paragraph = firstElement(node, "p");
      const marker = paragraph?.children?.find(
        child => child.type === "text" && typeof child.value === "string",
      );
      const match = marker?.value.match(alertMarker);
      if (!match) return;

      const type = match[1].toLowerCase();
      const rest = marker.value.slice(match[0].length).replace(/^\n+/, "");
      return {
        type: "element",
        tagName: "div",
        properties: { className: ["markdown-alert", `markdown-alert-${type}`], dir: "auto" },
        children: [alertTitle(type), ...alertBody(node, paragraph, marker, rest)],
      };
    },
  },
};

export function siteMarkdownProcessor() {
  return satteri({
    features: {
      math: true,
      gfm: {
